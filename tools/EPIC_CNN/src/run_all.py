    ...
    for test_ratio in test_ratio_ls:
        for negative_ratio in negative_ratio_ls:
            for OUT_PREFIX in OUT_PREFIXS:
                ...
                exec_command = 'python ../../main.py -s '+FEATURES+' '+EP_DIR+OUT_PREFIX+' -c '+GD_FILE+' /home/kuan-hao/EPPC/output/'+OUT_DIR_SRC+'/'+OUT_PREFIX+'/'+OUT_DIR_MTHD+'/'+OUT_DIR_TRN+'/'+OUT_PREFIX+'__K_D__'+K_D_TRAIN+'__'+OUT_DIR_MTHD+'__num_ep_'+str(NUM_EP)+'__num_frc_'+str(NUM_FRC)+'__fold_number_'+str(FOLD_NUM)+'__negative_ratio_'+str(negative_ratio)+'__test_ratio_'+str(test_ratio)+' -o '+OUT_PREFIX+' -M CNN -n 10 -m EXP -f STRING --LEARNING_SELECTION '+str(LEARNING_SELECTION)+' --K_D_TRAIN '+str(K_D_TRAIN)+' --FOLD_NUM '+str(FOLD_NUM)+' --TRAIN_TEST_RATIO '+str(float(test_ratio)/100)+' --POS_NEG_RATIO '+str(negative_ratio) + ' --NUM_EP '+str(NUM_EP)+' --NUM_FRC '+str(NUM_FRC)+' --CNN_ENSEMBLE '+str(CNN_ENSEMBLE)
