...
PrInCE = function(profiles, gold_standard, 
  ...
  message("Check gold standard input ")
  ...
  message("Get features for each matrix separately ")
  features <- list()
  for (replicate_idx in seq_along(profiles)) {
    if (verbose) {
    ...
    message("Filter matrix based on Gaussianss")
