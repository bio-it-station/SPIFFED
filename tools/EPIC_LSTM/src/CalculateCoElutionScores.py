...
from keras.layers import Dense, Conv1D, Conv2D, MaxPooling2D, Flatten, Activation, Dropout, LSTM
...
class CalculateCoElutionScores():
	...
	def calculateScores(self, toPred, tokeep=set([])):
		...
		print("self.scores.shape: ", self.scores.shape)
		print("self.scores: ", self.scores[0])
		self.scores = self.scores.transpose(0, 2, 1)
		print("self.scores.shape: ", self.scores.shape)
		print("self.scores: ", self.scores[0])
...
def LSTM_raw_ef_model(num_ef, num_frc):
	model = Sequential()
	# add model layers
	model.add(LSTM(128, input_shape=(num_frc, num_ef), activation='relu', return_sequences=True))
...
class CLF_Wrapper:
	...
	def __init__(self, num_cores, classifier_select="RF", useFeatureSelection= False, num_ef=2, num_frc=60):
		...
		elif classifier_select == "LSTM":
			print("Using LSTM")
			thisCLF = LSTM_raw_ef_model(num_ef, num_frc)
	...
	def cv_eval(self, data, targets, folds= 5):
		...
		i = 1
		test_size = 0.3
		x_train, x_test, y_train, y_test = train_test_split(data, targets, test_size=test_size, random_state=0)
		...
		num_ef = 2
		num_frc = 27
		self.clf = LSTM_raw_ef_model(num_ef, num_frc)
		self.clf.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
		...
		#
		#
		# return [precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc, probs, preds, this_targets]
...
# y_pred_test_probas_whole = np.vstack(((1-y_pred_test_probas)[:,0], y_pred_test_probas[:,0])).T
	...
	def predict(self, toPred):
		...
		positive_cutoff = 0.5
		preds[preds >= positive_cutoff] = 1
		preds[preds < positive_cutoff] = 0
# y_pred_test_probas_whole = np.vstack(((1-y_pred_test_probas)[:,0], y_pred_test_probas[:,0])).T
