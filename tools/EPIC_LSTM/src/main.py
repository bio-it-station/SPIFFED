...
def main():
	...
	scoreCalc_pkl_path = "/Users/chaokuan-hao/Documents/BIO_IT_Station/output/ppi/clf_RAW_1_LSTM_intensity_H_LSTM_128_Dr_20_LSTM_128_Dr_20_Dense_32_20_10_5_1.pkl"
	...
	clf = CS.CLF_Wrapper(args.num_cores, classifier_select, num_ef=num_ef, num_frc=num_frc)
	...
	# scoreCalc_pkl_path = "/Users/chaokuan-hao/Documents/BIO_IT_Station/output/ppi/clf_RAW_1_LSTM_intensity_H_LSTM_128_Dr_20_LSTM_128_Dr_20_Dense_32_20_10_5_1.pkl"
	# scoreCalc_pkl_path = "/Users/chaokuan-hao/Documents/BIO_IT_Station/output/ppi/test.pkl"
	# with open(scoreCalc_pkl_path, 'wb') as f:  # Python 3: open(..., 'wb')
	#     pickle.dump(scoreCalc, f)

	# clf_pkl_path = "/Users/chaokuan-hao/Documents/BIO_IT_Station/output/ppi/clf_BeadsA.pkl"
	# with open(clf_pkl_path, 'wb') as f:  # Python 3: open(..., 'wb')
	#     pickle.dump(clf, f)
	#
	# gs_pkl_path = "/Users/chaokuan-hao/Documents/BIO_IT_Station/output/ppi/gs_BeadsA.pkl"
	# with open(gs_pkl_path, 'wb') as f:  # Python 3: open(..., 'wb')
	#     pickle.dump(gs, f)
	...
	# print("final_network: ", final_network)

	p_1 = [i.split('\t')[0] for i in final_network]
	p_2 = [i.split('\t')[1] for i in final_network]
	ppi_scores = [i.split('\t')[2] for i in final_network]
	# ppi_scores = np.array(ppi_scores).astype(float)
	#
	# scaled_ppi_scores = np.interp(ppi_scores, (ppi_scores.min(), ppi_scores.max()), (0.5, 1))
	#
	# ppi_scores = np.array(scaled_ppi_scores).astype('str')
	final_network = [a+'\t'+b+'\t'+c for a, b, c in zip(p_1, p_2, ppi_scores)]
	# print("final_network: ", final_network)
	print >> outFH, "\n".join(final_network)
	outFH.close()
