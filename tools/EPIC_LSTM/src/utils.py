...
def cv_bench_clf(scoreCalc, clf, gs, outDir, verbose=False, format="pdf", folds = 5):
	...
	this_targets, preds, probs, precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc = clf.cv_eval(data, targets, folds)
...
def predictInteractions(scoreCalc, clf, gs, to_train=True, verbose= True):
	...
	num_samples, num_frc, num_ef  = data_train.shape
	...
	print("data_train.shape: ", data_train.shape)
	# num_ppi, num_frc, num_ef = data_train.shape

	def getPredictions(scores, edges, clf):
		out = []

		num_samples, num_frc, num_ef  = scores.shape
		# for i in range(100):
		# 	print(scores[i])
	...
	out = []
	tmpscores = np.zeros((100000, num_frc, num_ef))
	edges = [""]*100000
	k = 0
	chunk_num=1
	scoreCalc.open()
	...
	for line in range(scoreCalc.to_predict):
		...
		edges[k] = edge
		# print("(edge_scores.shape)[0]: ", (edge_scores.shape)[0])
		tmpscores[k,0:(edge_scores.shape)[0]] = edge_scores
		k += 1
	# scoreCalc.close()
	out.extend(getPredictions(tmpscores[0:k,:], edges[0:k], clf))
	return out
