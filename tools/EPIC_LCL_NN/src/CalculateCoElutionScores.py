...
def intrpl(a, intrpl_sz):
	x = np.linspace(0, len(a)-1, num=len(a), endpoint=True)
	x_new = np.linspace(0, len(a)-1, num=(intrpl_sz+1)*(len(a)-1)+1, endpoint=True)
	f = interp1d(x, a, kind="linear")
	a_new = f(x_new)
	return a_new
...
def p2p_score_calculate(intrpl_ef_a, intrpl_ef_b, frc_sz, intrpl_sz):
	frac_scores = []
	window_sz = 1
...
# @ author Florian Goebels
# return Pearson correlation of two proteins
def calculateScore_LocalPcc(a,b):
	frc_sz = len(a)
	intrpl_sz = 5
	a_intrpl = intrpl(a, intrpl_sz)
	b_intrpl = intrpl(b, intrpl_sz)
	frac_scores = p2p_score_calculate(a_intrpl, b_intrpl, frc_sz, intrpl_sz)
	# print("@@@@frac_scores: ", frac_scores)
	return frac_scores

class LocalPcc:
	calculateScore = staticmethod(calculateScore_LocalPcc)
...
class CalculateCoElutionScores():
	...
	def __init__(self, scores, elutionData, outF, num_cores, cutoff = 0.5, verbose=True):
		...
		self.raw_efs = np.array([])
		self.raw_efs_norm = np.array([])
	...
	def calculateScores(self, toPred, tokeep=set([])):
		...
		self.scores = np.zeros((num_rows, num_frc))
		...
		for ppi in toPred:
			...
			if k % 100000 == 0:
				...
				write_buffer = ""
			i = 0
			# print("self.elutionData: ", len(self.elutionData))
			for ed in self.elutionData:
			...
			for ed in self.elutionData:
				...
				# if score == 0:
				# ppi_frc_sz.append(len(score))

				# Condition 1: Check all elements are zero
				# is_all_zero = np.all((score == 0))

				# Condition 2: Check more than x elements are not zero
#				num_of_non_zeros = np.count_nonzero(score)
#				is_less = num_of_non_zeros < 4

#				ppi_less.append(is_less)
				ppi_scores.extend(score)
#				print("*** score: ", score)
#				print("*** is_less: ", is_less)
#				print("ppi_scores.shape: ", len(ppi_scores))

#				print("ppi_scores: ", ppi_scores)
				# ppi_scores = np.append(ppi_scores, score)
				# print("ppi_scores.shape: ", ppi_scores.shape)
				# elif not hasattr(score, "__len__") and score == 0:
				# 	score = np.zeros((2, len(ed.elutionMat[0])))
				# 	# print("HAHAHAHAHA")
				# 	print("score: ", score)
				# 	print("   @@* scores.shape: ", score.shape)
...
#			if any(ppi_less):
#				ppi_scores = np.zeros(len(ppi_scores))
#			print("*** ppi_less: ", ppi_less)
#			print("*** ppi_scores: ", ppi_scores)
#			print("ppi_scores.shape: ", len(ppi_scores))
			# print("**ppi_scores.shape: ", ppi_scores.shape)

			# for _ in range(num_features):
			# 	# score_index, ppi_scores = out_queue.get()
			# 	score_index, ppi_scores = out_queue.get()
			...
			if len(ppi_scores[np.where(ppi_scores > 0)]) > 3:
#			if len(ppi_scores[np.where(ppi_scores != 0)]) > 0:
				# print("Inside!!!")
				self.to_predict += 1
	...
	def filter_interactions(self, eData, ppis):
		...
		for ppi in ppis:
			...
			# protA_ep = eData.getElution(protA)
			# protB_ep = eData.getElution(protB)

			# if not (len(protA_ep[np.where(protA_ep > 0)]) < 5 or len(protB_ep[np.where(protB_ep > 0)]) < 5):
			# 	out.add(ppi)
...
# 	outputList.put((score_index, score))
		...
		for elution_data in self.elutionData:
			...
			# counter = 0
			# for ppi in filteredPPIs:
			# 	ppi_list = ppi.split("\t")
			# 	# print(ppi_list[0])
			# 	# print(ppi_list[1])
			# 	prot_1_unnorm = elution_data.getElution(ppi_list[0], normed=False)
			# 	prot_1_norm = elution_data.getElution(ppi_list[0], normed=True)
			# 	prot_2_unnorm = elution_data.getElution(ppi_list[1], normed=False)
			# 	prot_2_norm = elution_data.getElution(ppi_list[1], normed=True)
			#
			# 	if self.raw_efs.size == 0:
			# 		self.raw_efs = np.array([[prot_1_unnorm, prot_2_unnorm]])
			# 	else:
			# 		self.raw_efs = np.vstack([self.raw_efs, np.array([[prot_1_unnorm, prot_2_unnorm]])])
			#
			# 	if self.raw_efs_norm.size == 0:
			# 		self.raw_efs_norm = np.array([[prot_1_norm, prot_2_norm]])
			# 	else:
			# 		self.raw_efs_norm = np.vstack([self.raw_efs_norm, np.array([[prot_1_norm, prot_2_norm]])])
			# 	print("counter: ", counter)
			# 	counter += 1

				# print(self.raw_efs)
				# print(self.raw_efs.shape)
...
def NN_raw_ef_model(num_ef, num_frc):
    model = Sequential()
    # add model layers
...
class CLF_Wrapper:
	...
	def __init__(self, num_cores, classifier_select="RF", useFeatureSelection= False, num_ef=2, num_frc=60):
		...
		elif classifier_select == "NN":
			print("Using CNN")
			thisCLF = NN_raw_ef_model(num_ef, num_frc)
	...
	def get_metrics(self, probs, preds, targets):
		...
		# skplt.metrics.plot_confusion_matrix(targets, preds, normalize=False)
		# plt.savefig(os.path.join(outDir, "Cross_Validation", "confusion_matrix.png"), dpi=400)
		# plt.close()
		#
		# print("     ** Plot normalized confusion_matrix: ")
		# skplt.metrics.plot_confusion_matrix(targets, preds, normalize=True)
		# plt.savefig(os.path.join(outDir, "Cross_Validation", "confusion_matrix_normalized.png"), dpi=400)
		# plt.close()
		#
		# print("     ** Plot curve_roc: ")
		# skplt.metrics.plot_roc(targets, probs_whole, classes_to_plot = [1], plot_micro=False, plot_macro=False)
		# plt.savefig(os.path.join(outDir, "Cross_Validation", "curve_roc.png"), dpi=400)
		# plt.close()
		#
		# print("     ** Plot curve_pr: ")
		# skplt.metrics.plot_precision_recall(targets, probs_whole, classes_to_plot = [1], plot_micro=False)
		# plt.savefig(os.path.join(outDir, "Cross_Validation", "curve_pr.png"), dpi=400)
		# plt.close()
		#
		return [preds, probs, precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc]
	...
	def cv_eval(self, data, targets, folds= 5):
		...
		for train, test in skf.split(data, targets):
			...
			num_ef = 1
			num_frc = 27
			self.clf = NN_raw_ef_model(num_ef, num_frc)
			self.clf.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
			self.fit(data[train], targets[train], data[test], targets[test])
