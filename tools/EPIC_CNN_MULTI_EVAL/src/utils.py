...
def predictInteractions(scoreCalc, scoreCalc_eval, clf, gs, gs_eval, outDir, to_train=True, verbose= True, k_d_training = "k", folds = 5, train_test_ratio = None):
	...
	outDir_testing = outDir + os.sep + "testing"
	...
	chunk_num=1
	scoreCalc.open()
	print "scoreCalc > to predict: %i" % scoreCalc.to_predict
	...
	##########################
	out_eval = []
	tmpscores_eval = np.zeros((100000, num_ep_eval_all, num_frc_eval_all))
	edges_eval = [""]*100000
	k_eval = 0
	chunk_num_eval=1
	scoreCalc_eval.open()
	...
	for line in range(scoreCalc_eval.to_predict):
		...
		# print("edge_eval: ", edge_eval)
		# print("edge_scores_eval: ", edge_scores_eval)
		...
		k_eval += 1
	# scoreCalc.close()
	# print("edges_eval[0:k_eval]: ", edges_eval[0:k_eval])
	out_eval.extend(getPredictions(tmpscores_eval[0:k_eval,:], edges_eval[0:k_eval], clf))
	##########################
	## End of testing
	##########################
...
def make_predictions(score_calc, score_calc_eval, mode, clf, gs, gs_eval, output_dir, fun_anno="", verbose = False, k_d_training = "k", folds = 5, train_test_ratio = None):
	...
	# 		sys.exit()
	# 	networks.append(predictInteractions(score_calc, score_calc_eval, clf, gs, gs_eval, output_dir, True, verbose, k_d_training, folds, train_test_ratio))
	#
