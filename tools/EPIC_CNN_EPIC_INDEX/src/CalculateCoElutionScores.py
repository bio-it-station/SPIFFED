...
def getScore(inputQ, outputList):
	# raw_eps_score_func = Raw_eps()
	for args in iter(inputQ.get, 'STOP'):
		global prot2profile
		# ppi, score_index, fname, scoreFun = args
		ppi, score_index, fname, ed_name, frc_sz, scoreFun = args
		...
		if protA in prot2profile[fname] and protB in prot2profile[fname]:
			...
			# print("scoreFun: ", scoreFun)
			# print("score: ", score)
			# print("ed_name: ", ed_name)
			raw_profileA = prot2profile[ed_name][protA]
			raw_profileB = prot2profile[ed_name][protB]
			# print("raw_profileA: ", raw_profileA)
			# print("raw_profileB: ", raw_profileB)
			# print("raw_eps: ", raw_eps)
			# raw_eps.
			# for prot in elutionData.prot2Index:
			# np.argmax(elutionData.getElution(protA))
			raw_eps_score = np.array([raw_profileA, raw_profileB])
			# print("*****")
			# print("raw_eps_score: ", raw_eps_score)
			# print("*****")

			# outputList.put((score_index, score))

			outputList.put((score_index, score, raw_eps_score))

		else:
			raw_eps_score = np.zeros((2, frc_sz))
			# outputList.put((score_index, 0.0))
			outputList.put((score_index, 0.0, raw_eps_score))
...
class CalculateCoElutionScores():
	...
	def calculateScores(self, toPred, tokeep=set([])):
		...
		self.ppiToIndex = {}
		num_rows = len(tokeep)
		num_features = (len(self.features)-1)*len(self.elutionData)
		num_frc = 0
		for ed in self.elutionData:
		...
		self.raw_ep_scores = np.zeros((num_rows, 2, num_frc))
		...
		for ppi in toPred:
			...
			for ed in self.elutionData:
				for score in self.features[:-1]:
					...
					task = (ppi, i, fname, ed.name, len(ed.elutionMat[0]), score.calculateScore)
					task_queue.put(task)
					i += 1
			task_queue.join()
			ppi_scores = [0]*num_features
			raw_ppi_scores = np.array([[],[]])

			for _ in range(num_features):
				# score_index, score = out_queue.get()
				score_index, score, raw_score = out_queue.get()
				ppi_scores[score_index] = score
				if hasattr(raw_score, "__len__"):
					raw_ppi_scores = raw_score
			# raw_ppi_scores = np.concatenate((raw_ppi_scores, raw_score), axis=1)
			# print("raw_ppi_scores: ", raw_ppi_scores)

			ppi_scores = np.nan_to_num(np.array(ppi_scores))
			raw_ppi_scores = np.nan_to_num(np.array(raw_ppi_scores))


			# if len(list(set(np.where(ppi_scores > self.cutoff)[0]))) > 0:
			if len(list(set(np.where(ppi_scores > 0)[0]))) > 0.5:
				self.to_predict += 1
				# write_buffer +=  "%s\t%s\n" % (ppi, "\t".join(map(str, ppi_scores)))
				write_buffer +=  "%s\t%s\n" % (ppi, "\t".join(map(str, raw_ppi_scores.flatten())))
				# print("**raw_ppi_scores: ", raw_ppi_scores)
				if ppi in tokeep:
					...
					self.raw_ep_scores[ppi_index,:] = raw_ppi_scores
		...
		# self.raw_ep_scores = self.raw_ep_scores[0:ppi_index,:]
		self.scores = self.raw_ep_scores[0:ppi_index,:]
		print("** self.scores.shape: ", self.scores.shape)
		print("** self.scores: ", self.scores)
		# print("** self.raw_ep_scores: ", self.raw_ep_scores.shape)
...
def CNN_raw_ef_model(num_ef, num_frc):
	model = Sequential()
	# add model layers
	model.add(Conv2D(128, kernel_size=(2,2), activation='relu', input_shape=(num_ef, num_frc,1)))
...
class CLF_Wrapper:
	...
	def __init__(self, num_cores, classifier_select="RF", useFeatureSelection= False, num_ef=2, num_frc=60):
	# def __init__(self, num_cores, forest=False, useFeatureSelection= False):
		...
		if classifier_select == "RF":
			...
			print("Using CNN")
			thisCLF = CNN_raw_ef_model(num_ef, num_frc)
	...
	def fit(self, data, targets, data_test, targets_test):
		# self.clf.fit(data, targets)
		early_stopping = EarlyStopping(monitor='val_loss', patience=50, verbose=2)
		self.clf.fit(data, targets, validation_data=(data_test, targets_test), epochs=200, callbacks=[early_stopping])

	def eval(self, data, targets):
		probs = self.predict_proba(data)
		preds = self.predict(data)
		return self.get_metrics(probs, preds, targets, probs, preds, targets)

	def get_metrics(self, probs, preds, targets, probs_all_pos_neg, preds_all_pos_neg, targets_all_pos_neg):
		...
		precision_all_pos_neg = metrics.precision_score(targets_all_pos_neg, preds_all_pos_neg, average=None)[1]
		recall_all_pos_neg = metrics.recall_score(targets_all_pos_neg, preds_all_pos_neg, average=None)[1]
		fmeasure_all_pos_neg = metrics.f1_score(targets_all_pos_neg, preds_all_pos_neg, average=None)[1]
		auc_pr_all_pos_neg = average_precision_score(targets_all_pos_neg, preds_all_pos_neg)
		auc_roc_all_pos_neg = roc_auc_score(targets_all_pos_neg, preds_all_pos_neg)
		curve_pr_all_pos_neg = precision_recall_curve(targets_all_pos_neg, probs_all_pos_neg)
		curve_roc_all_pos_neg = roc_curve(targets_all_pos_neg, probs_all_pos_neg)
		return [targets, preds, probs, precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc, targets_all_pos_neg, preds_all_pos_neg, probs_all_pos_neg, precision_all_pos_neg, recall_all_pos_neg, fmeasure_all_pos_neg, auc_pr_all_pos_neg, auc_roc_all_pos_neg, curve_pr_all_pos_neg, curve_roc_all_pos_neg]
	...
	def cv_eval(self, data, targets, folds= 5):
		...
		# probs_all_pos_neg = []
		# preds_all_pos_neg = []
		# this_targets_all_pos_neg = []
		# probs_all_pos_neg.extend(self.predict_proba(data))
		# preds_all_pos_neg.extend(self.predict(data))
		# this_targets_all_pos_neg.extend(targets)
		#
		...
		num_ef = 2
		num_frc = 27
		self.clf = CNN_raw_ef_model(num_ef, num_frc)
		self.clf.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
		self.fit(x_train, y_train, x_test, y_test)
		probs.extend(self.predict_proba(x_test))
		preds.extend(self.predict(x_test))
		this_targets.extend(y_test)
	...
	def cv_eval_all(self, data, targets, folds= 5):
		probs_all_pos_neg = []
		preds_all_pos_neg = []
		this_targets_all_pos_neg = []
		probs_all_pos_neg.extend(self.predict_proba(data))
		...
		return self.get_metrics(probs_all_pos_neg, preds_all_pos_neg, this_targets_all_pos_neg, probs_all_pos_neg, preds_all_pos_neg, this_targets_all_pos_neg)
