...
def cv_bench_clf(scoreCalc, clf, gs, outDir, verbose=False, format="pdf", folds = 5):
	...
	this_targets, preds, probs, precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc, this_targets_all, preds_all, probs_all, precision_all, recall_all, fmeasure_all, auc_pr_all, auc_roc_all, curve_pr_all, curve_roc_all = clf.cv_eval(data, targets, folds)
	...
	this_targets_, preds_, probs_, precision_, recall_, fmeasure_, auc_pr_, auc_roc_, curve_pr_, curve_roc_, this_targets_all_, preds_all_, probs_all_, precision_all_, recall_all_, fmeasure_all_, auc_pr_all_, auc_roc_all_, curve_pr_all_, curve_roc_all_ = clf.cv_eval_all(data_all, targets_all, folds)
...
def predictInteractions(scoreCalc, clf, gs, to_train=True, verbose= True):
	...
	num_ppi, num_ef, num_frc, tmp = data_train.shape
	...
	out = []
	tmpscores = np.zeros((100000, num_ef, num_frc))
	edges = [""]*100000
	k = 0
	chunk_num=1
	scoreCalc.open()
