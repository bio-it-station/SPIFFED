...
def cv_bench_clf(scoreCalc, clf, gs, outDir, verbose=False, format="pdf", folds = 5):
	...
	data = data.reshape((num_samples, num_frc))
	print("data.shape: ", data.shape)
	print(data[0])
	print("targets.shape: ", targets.shape)
	print(targets)

	preds, probs, precision, recall, fmeasure, auc_pr, auc_roc, curve_pr, curve_roc = clf.cv_eval(data, targets, folds)
	...
	skplt.metrics.plot_roc(targets, probs_concat, classes_to_plot = [1], plot_micro=False, plot_macro=False)
	plt.savefig(os.path.join(outDir, "curve_roc.png"), dpi=400)
	plt.close()

	print("     ** Plot curve_pr: ")
	skplt.metrics.plot_precision_recall(targets, probs_concat, classes_to_plot = [1], plot_micro=False)
...
def predictInteractions(scoreCalc, clf, gs, to_train=True, verbose= True):
	...
	data_train = data_train.reshape((num_samples, num_frc))
	print("data_train.shape: ", data_train.shape)

	# if to_train:
	# 	data_train_train, data_train_test, targets_train_train, targets_train_test = train_test_split(data_train, targets_train, test_size=0.3)
	# 	# # clf.fit(data, targets, data_test, targets_test)
	# 	clf.fit(data_train_train, targets_train_train, data_train_test, targets_train_test)
	#
	# 	# early_stopping = EarlyStopping(monitor='val_loss', patience=50, verbose=2)
	# 	# clf.fit(data_train_train, targets_train_train, validation_data=(data_train_test, targets_train_test), epochs=200, callbacks=[early_stopping])
	# 	# self.clf.fit(data, targets, validation_data=(data_test, targets_test), epochs=200, callbacks=[early_stopping])

	print("data_train.shape: ", data_train.shape)
	num_ppi, num_frc = data_train.shape

	def getPredictions(scores, edges, clf):
		out = []

		# y_pred_test_probas = clf.predict(data[test])
		...
		num_samples, num_frc = scores.shape
		# print("data: ", data.shape)
		scores = scores.reshape((num_samples, num_frc))
	...
	out = []
	tmpscores = np.zeros((100000, num_frc))
	edges = [""]*100000
	k = 0
	chunk_num=1
	scoreCalc.open()
